"""
Enhanced training script for ResNet with comprehensive per-class metrics.
"""
...
def residual_block(x, filters, kernel_size=3, stride=1):
    ...
    """
    # Main path
    fx = layers.Conv1D(filters, kernel_size, strides=stride, padding='same')(x)
    fx = layers.BatchNormalization()(fx)
    fx = layers.Activation('relu')(fx)
    fx = layers.Dropout(0.2)(fx)
    
    fx = layers.Conv1D(filters, kernel_size, strides=1, padding='same')(fx)
    fx = layers.BatchNormalization()(fx)
...
class ResNetFileClassifier:
    """ResNet classifier for file type identification."""
...
def main():
    ...
    print("="*80)
    print("RESNET FILE TYPE CLASSIFIER")
    print("="*80)
    print()
    ...
    X, y, file_types, loader = load_train_data(
        ...
        normalize=True,  # ResNet needs normalization
        verbose=True
    )
